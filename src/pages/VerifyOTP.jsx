...
import { FaEnvelope, FaMobileAlt, FaWhatsapp, FaLock } from 'react-icons/fa';
...
const VerifyOTP = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await api.post('/auth/user/verify', {
        requestId,
        otpCode
      });
      ...
      if (res.data.success && res.data.verified) {
        ...
        console.log('Redirecting to End User Dashboard');
  ...
  const getChannelIcon = () => {
    switch (channel) {
      case 'email':
        return <FaEnvelope className="text-5xl mx-auto" />;
      case 'sms':
        return <FaMobileAlt className="text-5xl mx-auto" />;
      case 'whatsapp':
        return <FaWhatsapp className="text-5xl mx-auto" />;
      default:
        return <FaLock className="text-5xl mx-auto" />;
    }
  };
